...
(function applyV25StudioPBR(){
  ...
  // Reference-like default composition: keep the complete engine inside the center stage
  // instead of starting from an over-close component crop. Mobile keeps the original camera.
  if(!isMobileView){
  ...
  function studioFace(kind){
    ...
    strip.addColorStop(.40,'rgba(255,255,255,.04)');
  ...
  frontFill.color.setHex(0xe9f5ff);frontFill.intensity=.34;frontFill.position.set(-120,430,1050);
  lowFill.intensity=.04;
  ...
  function tunePhysical(m,opt){
    ...
    if('envMapIntensity' in m)m.envMapIntensity=opt.envMapIntensity===undefined?1.25:opt.envMapIntensity;
    m.dithering=true;m.needsUpdate=true;
  }

  // Main visible V20 digital-twin palette. Roughness is deliberately separated so
  // cast iron, machined steel and warm copper/brass do not collapse into one glossy plastic look.
  ...
  tunePhysical(refM.machined,{color:0xe5e9eb,roughness:.105,metalness:1,clearcoat:.38,clearcoatRoughness:.11,envMapIntensity:1.58});
  ...
  if(typeof m22!=='undefined'){
    ...
    tunePhysical(m22.steel,{color:0xd9dfe2,roughness:.12,metalness:1,clearcoat:.34,clearcoatRoughness:.11,envMapIntensity:1.54});
  ...
  scene.traverse(o=>{
    ...
    for(const m of mats){
      ...
      if('envMapIntensity' in m && m.envMapIntensity===1)m.envMapIntensity=m.transparent?.40:(m.metalness>.75?1.26:.66);
      m.dithering=true;m.needsUpdate=true;
    }
  });
  ...
  const sg=sx.createRadialGradient(256,132,12,256,132,238);sg.addColorStop(0,'rgba(0,0,0,.90)');sg.addColorStop(.42,'rgba(0,0,0,.46)');sg.addColorStop(.78,'rgba(0,0,0,.11)');sg.addColorStop(1,'rgba(0,0,0,0)');sx.fillStyle=sg;sx.fillRect(0,0,512,256);
  const shadowTex=new THREE.CanvasTexture(sc);shadowTex.minFilter=THREE.LinearFilter;shadowTex.magFilter=THREE.LinearFilter;
  const contactShadow=new THREE.Mesh(new THREE.PlaneGeometry(920,500),new THREE.MeshBasicMaterial({map:shadowTex,transparent:true,opacity:.68,depthWrite:false,color:0x000000}));
  ...
  if(!vignette){vignette=document.createElement('div');vignette.id='v25StudioVignette';vignette.style.cssText='position:absolute;inset:0;pointer-events:none;z-index:1;background:radial-gradient(ellipse at 52% 43%,rgba(0,0,0,0) 38%,rgba(0,0,0,.06) 62%,rgba(0,0,0,.42) 100%)';viewport.appendChild(vignette);}
  ...
  window.__v25VisualDebug={
    enabled:true,environment:!!scene.environment,exposure:renderer.toneMappingExposure,
    machinedRoughness:refM.machined.roughness,machinedEnv:refM.machined.envMapIntensity,
    castRoughness:refM.cast.roughness,brassMetalness:refM.brass.metalness,
    contactShadow:!!contactShadow,vignette:!!vignette,
    cameraPosition:camera.position.toArray(),cameraTarget:controls.target.toArray(),cameraFov:camera.fov
  };
})();